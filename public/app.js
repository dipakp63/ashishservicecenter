    ...
    try {
      ...
      readings.forEach(r => {
        ...
        if (nozzleBody) {
          ...
          tr.innerHTML = `
            <td style="padding: 0.2rem 0.35rem;"><span class="nozzle-badge">N${r.nozzle_id}</span></td>
            <td style="padding: 0.2rem 0.35rem;">${r.product}</td>
            <td style="text-align: right; padding: 0.2rem 0.35rem;">${opening.toFixed(3)}</td>
            <td style="text-align: right; padding: 0.2rem 0.35rem;">${closing.toFixed(3)}</td>
            <td style="text-align: right; padding: 0.2rem 0.35rem;">${diff.toFixed(3)}</td>
            <td style="text-align: right; padding: 0.2rem 0.35rem;">${testing.toFixed(3)}</td>
            <td style="text-align: right; font-weight: bold; padding: 0.2rem 0.35rem;">${net.toFixed(3)}</td>
          `;
      ...
      if (tankBody) {
        ...
        tanks.forEach(t => {
          ...
          const tr = document.createElement('tr');
          tr.innerHTML = `
            <td style="padding: 0.2rem 0.35rem;"><span class="nozzle-badge">T${t.tank_id}</span></td>
            <td style="padding: 0.2rem 0.35rem;">${t.tank_name} (${t.product})</td>
            <td style="text-align: right; padding: 0.2rem 0.35rem;">${(t.opening_dip || 0).toFixed(1)}</td>
            <td style="text-align: right; padding: 0.2rem 0.35rem;">${(t.opening_stock || 0).toFixed(2)}</td>
            <td style="text-align: right; padding: 0.2rem 0.35rem;">${decantation.toFixed(0)}</td>
            <td style="text-align: right; padding: 0.2rem 0.35rem;">${(t.closing_dip || 0).toFixed(1)}</td>
            <td style="text-align: right; font-weight: bold; padding: 0.2rem 0.35rem;">${(t.closing_stock || 0).toFixed(2)}</td>
          `;
      ...
      if (nonCashBody) {
        ...
        if (nonCash && nonCash.length > 0) {
          nonCash.forEach(p => {
            const tr = document.createElement('tr');
            tr.innerHTML = `
              <td style="padding: 0.2rem 0.35rem;">${p.type}</td>
              <td style="padding: 0.2rem 0.35rem;">${resolveDescription(p.description)}</td>
              <td style="text-align: right; padding: 0.2rem 0.35rem;">${p.amount.toFixed(2)}</td>
            `;
